...
async function compile() {
  ...
  for (const [src, dest] of files) {
    ...
    if (result.status !== 0) {
      console.error("Compile error:", result.stderr.toString());
      process.exit(1);
    }
  }
}
...
async function main() {
  console.log("Compiling WebSocket client and tests...");
  await compile();
  console.log("Running tests...");
  const result = spawnSync("node", ["--experimental-vm-modules", join(outDir, "webSocketClient.test.js")], { shell: true });
  console.log(result.stdout?.toString() || "");
  if (result.stderr?.toString()) console.error(result.stderr.toString());
